...
# --- 1. Configuration ---
SQL_MODEL_NAME = "Snowflake/Arctic-Text2SQL-R1-7B"
...
def analyze_results(question: str, sql_query: str, sql_result: dict) -> Tuple[str, float]:
    ...
    # Use full_data if available, fallback to data if not
    full_data = sql_result.get("full_data", sql_result.get("data", []))
    
    prompt = make_analysis_prompt(question, sql_query, {"data": full_data})
...
def execute_sql(sql_query: str) -> Tuple[Dict[str, Any], float]:
    ...
    try:
        ...
        # Save original data for internal use
        full_data = result.get("data", [])
        
        # Modify result to return only first 3 rows in final output
        if result.get("success") and isinstance(result.get("data"), list):
            result["data"] = result["data"][:3]  # Only show 3 rows in response
    ...
    # Attach full data back for internal processing
    result["full_data"] = full_data  # This will be used later by Phi model
